...
class ImageContainer:
	...
	def _get_job_status(self,timeout=900.0):
	...
	def _replace_new_wcs(self,delnew=True):
		...
		for c in tmp2.ascardlist():
                   try:
                        tmp[0].header.update(c.key,c.value)
...
class AstrometrySolver:
	...
	job_dict = {"uname": "Brad Cenko","email": "cenko@astro.berkeley.edu", "fsunit" :"arcsecperpix",\
		"fstype-ul": 1, "fsu": 0.30, "fsl": 0.35, "xysrc": "img", "parity": 2, "tweak": 0,\
		"tweak_order": 0, "imgfile": "","submit": "Submit"}

	def __init__(self,verbose=True):
		self.verbose= verbose
		self.threads = []
		
	def _make_request(self,imgfile=None,pixel_size_range = [0.30,0.35], tweak_astrometry=False):
