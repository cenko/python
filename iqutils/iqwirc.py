...
######################################################################
# $Id: iqwirc.py,v 1.2 2005/02/24 19:39:16 derekfox Exp derekfox $
######################################################################
...
def wirc_anet(image):
    ...
    root = image.split('.')[0]
    anetcmd = "anet_wirc.py %s > %s.wcs" % (image, root)
    print anetcmd
    success = 0
    while not success:
        os.system(anetcmd)
...
class wircred:
    ...
    def __init__(self,inpat="wirc????.fits",final=yes,
                 ...
                 #onofflist=['Ks','K'],bpmroot="BPM",
                 onofflist=[],bpmroot="BPM",
                 #bpmffilt="On-Ks,Off-Ks,On-H,On-J",
                 bpmffilt="On-J,On-H,On-Ks,Off-Ks",
                 flatpfx="f",fringepre="Fringe-",coaddkey="COADDS",
                 exptmkey="EXPTIME",defrgpfx="s",frgamp=0.25,
                 clipreg="[1:2020,1:2020]",clpsfx="-cut",
    ...
    def preproc(self,qombos):
        ...
        for image in allfiles:
            ...
            skybkg=wirc_sky(image,'!BPM')
            update_head(image,'SKYBKG',skybkg,
                        'Value of sky background')

            # Rudimentary WCS
            iraf.add_wcs(image,instrument="wirc")
...
def iqwirc(inpat="wirc????.fits",final=yes,
    ...
    # Define the wircred object
    wred=wircred(inpat=inpat,final=final,lampon=lampon,lampoff=lampoff,
...
def main():
    ...
    iqwirc(inpat=inpat,final=final,forcecals=forcecals,
