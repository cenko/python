...
def lmi_detrend(imlist, otype="OBJECT", ppre="p", bkey="BIASSEC", 
		        ...
		        fpre="f", flatpre="Flat", skybkg="SKYBKG", skysub="SKYSUB",
		        skysig="SKYSIG", fpfx="F", clobber=globclob, verbose=globver):
...
def lmi_stats(imlist, outf):
	...
	for im in ims:
		...
		obj = hdr[0].header["OBJECT"]; filt = hdr[0].header["FILTER"]
...
def lmi_defringe(imlist, filts=["SDSS-Z"], fkey="FILTER", skybkg="SKYBKG",
				 skysub="SKYSUB", fpfx="F"):
...
def lmi_plots(imlist, dfile):
	...
	for line in lines:
		...
		# Limiting magnitude
		ax3.plot(t.mjd, float(temp[10]), SEEDICT[temp[3]])
